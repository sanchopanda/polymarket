"""
Скачивает активные T20 (International Cricket) рынки с Polymarket и Kalshi.

Временно́е окно конкретного матча (из API):
  PM gameStartTime:              2026-03-29 09:30 UTC
  Kalshi expected_expiration:    2026-03-29 12:30 UTC
...
Команды (проверено для KXT20MATCH-26MAR28SRHRCB):
  PM outcomes:       ["Seychelles", "Eswatini"]
  Kalshi yes_sub_title: "Seychelles", "Eswatini"
  Токены ≥4 символов: "seychelles","eswatini" — TennisMatcher ✓

Сохраняет:
  data/pm_t20.json             — урезанные поля PM
  data/kalshi_t20.json         — урезанные поля Kalshi
  data/pm_t20_titles.json      — slug + question (для быстрого просмотра)
  data/kalshi_t20_titles.json  — event_ticker + title

Usage:
    # Конкретная дата матча из API (Шаг 1 флоу):
    python3 scripts/dump_t20_markets.py --date 2026-03-29T09:30:00+00:00

    # Широкое окно для обзора всех доступных игр:
    python3 scripts/dump_t20_markets.py --hours 72
"""
...
PM_SERIES_SLUG = "international-cricket"
KALSHI_SERIES = "KXT20MATCH"
...
def _slim_pm(m: dict) -> dict:
    ...
    return {
        ...
        "url":              f"https://polymarket.com/sports/crict20/{m.get('slug', '')}",
    }
...
def fetch_pm_t20(since: datetime, cutoff: datetime) -> tuple[list[dict], list[dict]]:
...
def fetch_kalshi_t20(since: datetime, cutoff: datetime) -> tuple[list[dict], list[dict]]:
    result_raw: list[dict] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Дамп T20 рынков")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--date", type=str,
                       help="gameStartTime матча из API (ISO, напр. 2026-03-29T09:30:00+00:00). "
    ...
    print("[PM] Скачиваем T20...")
    pm_slim, _ = fetch_pm_t20(since, cutoff)
    (OUT_DIR / "pm_t20.json").write_text(json.dumps(pm_slim, ensure_ascii=False, indent=2))
    pm_titles = [{"slug": m.get("slug"), "question": m.get("question"),
                  "game_start_time": m.get("game_start_time")}
                 for m in pm_slim]
    (OUT_DIR / "pm_t20_titles.json").write_text(json.dumps(pm_titles, ensure_ascii=False, indent=2))
    print(f"[PM] {len(pm_slim)} рынков → pm_t20.json, pm_t20_titles.json")
    ...
    ka_slim, _ = fetch_kalshi_t20(since, cutoff)
    (OUT_DIR / "kalshi_t20.json").write_text(json.dumps(ka_slim, ensure_ascii=False, indent=2))
    ...
    (OUT_DIR / "kalshi_t20_titles.json").write_text(json.dumps(ka_titles, ensure_ascii=False, indent=2))
    print(f"[Kalshi] {len(ka_slim)} рынков, {len(ka_titles)} событий → kalshi_t20.json, kalshi_t20_titles.json")
