"""
Скачивает активные IPL (Indian Premier League Cricket) рынки с Polymarket и Kalshi.

Временно́е окно конкретного матча (из API):
  PM gameStartTime:              2026-03-28 14:00 UTC
  Kalshi expected_expiration:    2026-03-28 17:00 UTC
...
Команды (проверено для KXIPLGAME-26MAR28SRHRCB):
  PM outcomes:       ["Royal Challengers Bengaluru", "Sunrisers Hyderabad"]
  Kalshi yes_sub_title: "Royal Challengers Bengaluru", "Sunrisers Hyderabad"
  Токены ≥4 символов: "royal","challengers","bengaluru","sunrisers","hyderabad" — TennisMatcher ✓

Сохраняет:
  data/pm_ipl.json             — урезанные поля PM
  data/kalshi_ipl.json         — урезанные поля Kalshi
  data/pm_ipl_titles.json      — slug + question (для быстрого просмотра)
  data/kalshi_ipl_titles.json  — event_ticker + title

Usage:
    # Конкретная дата матча из API (Шаг 1 флоу):
    python3 scripts/dump_ipl_markets.py --date 2026-03-28T14:00:00+00:00

    # Широкое окно для обзора всех доступных игр:
    python3 scripts/dump_ipl_markets.py --hours 72
"""
...
PM_SERIES_SLUG = "indian-premier-league"
KALSHI_SERIES = "KXIPLGAME"
...
def _slim_pm(m: dict) -> dict:
    ...
    return {
        ...
        "url":              f"https://polymarket.com/sports/cricipl/{m.get('slug', '')}",
    }
...
def fetch_pm_ipl(since: datetime, cutoff: datetime) -> tuple[list[dict], list[dict]]:
...
def fetch_kalshi_ipl(since: datetime, cutoff: datetime) -> tuple[list[dict], list[dict]]:
    result_raw: list[dict] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Дамп IPL рынков")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--date", type=str,
                       help="gameStartTime матча из API (ISO, напр. 2026-03-28T14:00:00+00:00). "
    ...
    print("[PM] Скачиваем IPL...")
    pm_slim, _ = fetch_pm_ipl(since, cutoff)
    (OUT_DIR / "pm_ipl.json").write_text(json.dumps(pm_slim, ensure_ascii=False, indent=2))
    pm_titles = [{"slug": m.get("slug"), "question": m.get("question"),
                  "game_start_time": m.get("game_start_time")}
                 for m in pm_slim]
    (OUT_DIR / "pm_ipl_titles.json").write_text(json.dumps(pm_titles, ensure_ascii=False, indent=2))
    print(f"[PM] {len(pm_slim)} рынков → pm_ipl.json, pm_ipl_titles.json")
    ...
    ka_slim, _ = fetch_kalshi_ipl(since, cutoff)
    (OUT_DIR / "kalshi_ipl.json").write_text(json.dumps(ka_slim, ensure_ascii=False, indent=2))
    ...
    (OUT_DIR / "kalshi_ipl_titles.json").write_text(json.dumps(ka_titles, ensure_ascii=False, indent=2))
    print(f"[Kalshi] {len(ka_slim)} рынков, {len(ka_titles)} событий → kalshi_ipl.json, kalshi_ipl_titles.json")
