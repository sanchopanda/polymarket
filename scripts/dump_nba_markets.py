"""
Скачивает активные NBA (Professional Basketball) рынки с Polymarket и Kalshi.

Временно́е окно конкретного матча (из API):
  PM gameStartTime:              2026-03-28 21:30 UTC
  Kalshi expected_expiration:    2026-03-29 00:30 UTC
...
Команды (проверено для KXNBAGAME-26MAR28DETMIN):
  PM outcomes:       ["Pistons", "Timberwolves"]
  Kalshi yes_sub_title: "Detroit", "Minnesota"
  NBAMatcher: "detroit"→{"pistons"}, "minnesota"→{"timberwolves"} — TennisMatcher ✓

Сохраняет:
  data/pm_nba.json             — урезанные поля PM
  data/kalshi_nba.json         — урезанные поля Kalshi
  data/pm_nba_titles.json      — slug + question (для быстрого просмотра)
  data/kalshi_nba_titles.json  — event_ticker + title

Usage:
    # Конкретная дата матча из API (Шаг 1 флоу):
    python3 scripts/dump_nba_markets.py --date 2026-03-28T21:30:00+00:00

    # Широкое окно для обзора всех доступных игр:
    python3 scripts/dump_nba_markets.py --hours 72
"""
...
def _slim_pm(m: dict) -> dict:
    ...
    return {
        ...
        "url":              f"https://polymarket.com/sports/nba/{m.get('slug', '')}",
    }
...
def fetch_pm_nba(since: datetime, cutoff: datetime) -> tuple[list[dict], list[dict]]:
        ...
        for _ in range(50):
            ...
            for m in page:
                ...
                if (since <= dt <= cutoff
                        ...
                        and (ev.get("seriesSlug") or "").startswith("nba-")):
                    result_raw.append(m)
...
def fetch_kalshi_nba(since: datetime, cutoff: datetime) -> tuple[list[dict], list[dict]]:
    result_raw: list[dict] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Дамп NBA рынков")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--date", type=str,
                       help="gameStartTime матча из API (ISO, напр. 2026-03-28T21:30:00+00:00). "
    ...
    print("[PM] Скачиваем NBA...")
    pm_slim, _ = fetch_pm_nba(since, cutoff)
    (OUT_DIR / "pm_nba.json").write_text(json.dumps(pm_slim, ensure_ascii=False, indent=2))
    pm_titles = [{"slug": m.get("slug"), "question": m.get("question"),
                  "game_start_time": m.get("game_start_time")}
                 for m in pm_slim]
    (OUT_DIR / "pm_nba_titles.json").write_text(json.dumps(pm_titles, ensure_ascii=False, indent=2))
    print(f"[PM] {len(pm_slim)} рынков → pm_nba.json, pm_nba_titles.json")
    ...
    ka_slim, _ = fetch_kalshi_nba(since, cutoff)
    (OUT_DIR / "kalshi_nba.json").write_text(json.dumps(ka_slim, ensure_ascii=False, indent=2))
    ...
    (OUT_DIR / "kalshi_nba_titles.json").write_text(json.dumps(ka_titles, ensure_ascii=False, indent=2))
    print(f"[Kalshi] {len(ka_slim)} рынков, {len(ka_titles)} событий → kalshi_nba.json, kalshi_nba_titles.json")
