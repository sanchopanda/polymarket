...
## Команды: `python3 -m weather_bot`

Погодный бот для Polymarket `highest-temperature-in-{city}-on-{date}` событий.
**Read-only pipeline**: тянет forecast + intraday observations из NWS, снимает
orderbook из Gamma (bestAsk/spread/liquidity), считает Gaussian p_model по
бакетам, пишет снэпшоты в БД. Торговля не включена.

- `scan` — одноразовый показ сигналов без записи в DB
- `tick` — один проход pipeline с записью в DB (для крона)
- `watch` — loop с интервалом `runtime.interval_seconds`
- `status` — сводка из DB + топ активных сигналов

Аналитические скрипты:

- `scripts/resolve_weather.py` — заполняет `outcomes` фактическими high для
  всех `markets.event_date < today_local`. Читает NWS
  `/stations/{icao}/observations` за local-day окно (с учётом TZ станции),
  берёт max температуры, находит выигрышный bucket. Идемпотентен. Запускать
  ежедневно после ~04:00 UTC (когда все US TZ уже закрыли local day).
- `scripts/analyze_weather.py` — полный анализ на резолвнутых event'ах:
  1. Forecast RMSE и bias per-city, per-lead (включая `lead<0` — intraday).
  2. Calibration p_model vs yes_price vs yes_best_ask (emp hit rate по bin'ам).
  3. Spread & liquidity profile + slippage mid-edge vs ask-edge.
  4. Backtest first-signal per bucket, **fill по best_ask** (buy-YES по
     `yes_best_ask`, buy-NO по `1-yes_best_bid`). Fallback на mid для старых
     снимков без orderbook-колонок.
  5. Intraday edge: сколько раз `observed_max_f > bucket.hi` уже делает
     бакет 100% проигрышным, пока рынок ещё котирует YES > 0.
  6. Strategy grid-search: (side × min_edge × max_spread × min_liq ×
     tail/middle × max_lead) → топ-10 по `avg_pnl × √n`.
  7. Per-city breakdown PnL.

Конфиг: `weather_bot/config.yaml`
База: `data/weather_bot.db`

Архитектура:

- `markets.py` — Gamma API `/events?tag_slug=daily-temperature&end_date_min=now`,
  парсинг бакетов из `groupItemTitle` (`"64-65°F"`, `"77°F or below"`,
  `"96°F or higher"`). Фильтрация по US-станциям из `stations.py`. `Bucket`
  включает `yes_best_ask`, `yes_best_bid` (= `bestAsk - spread`), `yes_spread`,
  `liquidity_num`, `volume_24h`, `last_trade_price` + midprice (`yes_price`).
  `no_best_ask_est = 1 - yes_best_bid` — оценка (NO-книга торгуется независимо
  на negRisk, но для backtest'а этого достаточно).
- `stations.py` — 11 US-городов со станцией резолва и IANA TZ. Polymarket
  резолвит по Weather Underground той же станции; для US данные совпадают с ASOS/NWS.
- `nws.py` — клиент api.weather.gov (без ключа, `User-Agent` обязателен).
  Кеширует grid-point по lat/lon.
  - `forecast_high(station, event_date)` — max из `/forecast/hourly` за local day.
  - `observed_max_so_far(station, event_date, now_utc)` — max из
    `/stations/{icao}/observations` за локальный день до `now_utc`; возвращает
    `(max_f | None, n_obs, hours_remaining)`. None до начала local day.
- `model.py` — Gaussian CDF по бакетам. `sigma = rmse_for_lead(lead_hours)`:
  2.5°F (<24h), 3.0°F (<48h), 3.5°F (<72h), 4.5°F (<96h). Поправка на
  округление: бакет [lo,hi] покрывает `[lo-0.5, hi+0.5]`.
- `engine.py` — loop. На тик: fetch events → upsert → на каждую уникальную
  (city,date): 1 forecast + 1 observed_max_so_far → snapshot per bucket.
  Все cache'ируются в рамках тика, чтобы NWS не получал 11 одинаковых запросов.
- `db.py` — SQLite с авто-миграцией через `ALTER TABLE ADD COLUMN` (идемпотентно).

База `data/weather_bot.db` — таблицы:

- `markets(event_id PK, slug, title, city_slug, station_icao, event_date, end_date_utc, first_seen_ts)`
- `buckets(id PK, event_id, title, lo, hi, yes_token_id, no_token_id)`
  - хвосты: `lo=-1e9` (or below) или `hi=1e9` (or higher)
- `forecasts(id PK, ts, city_slug, event_date, mu_f, sigma_f, lead_hours, source, hours_covered)`
- `snapshots(id PK, ts, bucket_id, forecast_id, p_model, yes_price, no_price, edge_yes,`
  ` yes_best_ask, yes_best_bid, yes_spread, liquidity_num, volume_24h, last_trade_price,`
  ` observed_max_f, hours_remaining)`
  - `yes_best_*` колонки добавлены миграцией — для снимков до миграции они NULL,
    анализ использует fallback на `yes_price`/`no_price`.
- `obs_max(id PK, ts, station_icao, event_date, observed_max_f, n_obs, hours_remaining,`
  ` UNIQUE(station_icao, event_date, ts))` — intraday observed-max-so-far,
  агрегирован per (station,date), не per bucket.
- `outcomes(event_id PK, actual_high_f, winning_bucket_id, resolved_ts)` — заполняется
  `scripts/resolve_weather.py`.

Операционные заметки:

- NWS accept-rate: 11 городов × 2 endpoints (forecast + observations) = 22 запроса
  на тик. При `interval_seconds=600` это ~3.7 запросов/мин — комфортно в рамках
  fair-use NWS.
- Если поменял код (bucket schema, engine.tick) — **перезапусти watch**, Python
  не перечитывает модули. Schema migration прозрачна: новые колонки создаются при
  следующем `WeatherDB.__init__`.
- Все значения температур в БД — °F (observed max из NWS конвертируется из °C).
- Polymarket negRisk: `yes_best_ask` у каждого bucket — это yes-side orderbook.
  NO-side мы не котируем, оценка `1 - yes_best_bid` — верхняя граница buy-NO fill'а.

План развития `weather_bot`:

1. ✅ Read-only сбор: forecast + orderbook + intraday observed_max.
2. ✅ Resolve + analyze скрипты (forecast RMSE, calibration, backtest по
   bestAsk, intraday-cut edge, grid-search).
3. 🔄 Набрать 50-100+ резолвнутых event'ов (1-2 недели × 11 городов) для
   статистической значимости. Пересчитать σ(lead) на своих данных, посмотреть
   per-city bias.
4. 🔜 Добавить фильтр по intraday observed-cut: если `observed_max_f > bucket.hi`
   и рынок ещё котирует YES ≥ 1¢ — это (почти) риск-free buy-NO. Стратегия,
   которая скорее всего заработает первой.
5. 🔜 Paper trading с отдельным `positions` / `pnl_ledger` по паттерну
   `oracle_arb → recovery_bot`. Только после положительного WR на реальных
   резолвах — реальные ордера с маленьким floor.
